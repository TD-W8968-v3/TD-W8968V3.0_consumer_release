...
extern void config_fac_default_irq(unsigned int irq, unsigned int high_level); /* function from irq.c by wwj 130228 */
...
static void __init wifi_board_init(void);  /* add for wifi btn by wwj 130228 */
#endif
...
/* added for restore default work by wwj 130228 */
static uint32 l_restoreDefaultFlag = 0;
struct timer_list g_reset_fac_timer;
static void doRestoreDefaultWork(unsigned long arg);
//static uint32 l_wifiSwitchFlag = 0;
/* end added */
...
/* add by wwj for wlan button scan 130408 */
static struct timer_list wlan_timer_t;
...
static int __init brcm_board_init( void )
{
    ...
    else
    {
        ...
        ses_board_init();
	wifi_board_init(); /* add by wwj 130228 */
	...
	/* add timer for restore default by wwj 130228 */	
	memset(&g_reset_fac_timer, 0, sizeof(struct timer_list));	
	init_timer(&g_reset_fac_timer);	
	g_reset_fac_timer.function = doRestoreDefaultWork;
...
/* add for wlan_timer func by wwj 130228 */
static void wlanBtn_fn(unsigned long arg)
{
	static int lockFlag = 0;
	static int wlanBtnState = 0;
	static int wlanLockFlag = 0;
	...
	if (FREQUENCY == lockFlag)
	{
		lockFlag = -1;
	}

	if (BpGetWirelessBtnGpio(&gpio) == BP_SUCCESS)
	{
	...
	if (-1 == lockFlag && -1 == wlanLockFlag)
	{
		/* control wifi switch */
		lockFlag = 0;
		wlanLockFlag = 0;
		kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_WIFI_SWITCH, NULL, 0);
	}
	
	mod_timer(&wlan_timer_t, jiffies + (2 * HZ) / FREQUENCY);
}
...
static void __init wifi_board_init(void)
{
	...
	init_timer(&wlan_timer_t);
	wlan_timer_t.function = wlanBtn_fn;
	wlan_timer_t.data = data;
	wlan_timer_t.expires = jiffies + (2 * HZ) / FREQUENCY;
...
static irqreturn_t reset_isr(int irq, void *dev_id)
{
    ...
    if (!value)
    {
		...
		/* modify to match up our reset button by wwj 130228 */
		if( BpGetResetToDefaultExtIntr(&rstToDflt_irq) == BP_SUCCESS )
		{
			rstToDflt_irq = map_external_irq(rstToDflt_irq);
		}

		if (0 == l_restoreDefaultFlag)
		{
			l_restoreDefaultFlag = 1;
			config_fac_default_irq(rstToDflt_irq, 1); /* set to hi level trigger */
			/* set a timmer to reset fac default */
			del_timer(&g_reset_fac_timer);
			g_reset_fac_timer.expires = jiffies + 5 * HZ; /* set to 5 sec to timeout */
			add_timer(&g_reset_fac_timer);
		}
		else
		{
			l_restoreDefaultFlag = 0;
			config_fac_default_irq(rstToDflt_irq, 0); /*set back to low level trigger */
		}
...
static void doRestoreDefaultWork(unsigned long arg)
{
    if (1 == l_restoreDefaultFlag)
    {
        kerSysLedCtrl(kLedSes, kLedStateFastBlinkContinues);
...
#define offset(type, elem) ((int)&((type *)0)->elem)
	...
	static int IsFactoryTest[2] = {offset(NVRAM_DATA, isFactoryTest), 1};
	static int rsaSig[2] = {offset(NVRAM_DATA, rsaSig), NVRAM_RSA_SIG_LEN};
	static int fs_kernel_len[2] = {offset(NVRAM_DATA, fs_kernel_len), NVRAM_FS_KERNEL_LEN};
	...
	p1 = create_proc_entry("WpsDevicePin", 0644, p0);
	if (p1 == NULL)
		return -1;
	
	p1->data = WpsDevicePin;
	p1->read_proc = proc_get_param;
	p1->write_proc = proc_set_param;
	...
	p1 = create_proc_entry("RsaSig", 0644, p0);
	if (p1 == NULL)
		return -1;
	...
	p1 = create_proc_entry("fs_kernel_len", 0644, p0);
	if (p1 == NULL)
		return -1;

	p1->data        = fs_kernel_len;
	p1->read_proc   = proc_get_param;
	p1->write_proc  = proc_set_param;
   ...
   /* added by wwj 130517 for factory test */
   p1 = create_proc_entry("IsFactoryTest", 0644, p0);
	if (p1 == NULL)
		return -1;
	
	p1->data = IsFactoryTest;
	p1->read_proc = proc_get_param;
	p1->write_proc = proc_set_param;
...
static int proc_set_param(struct file *f, const char *buf, unsigned long cnt, void *data)
{
    ...
    if ((cnt > 258) || (copy_from_user(input, buf, cnt) != 0))
        return -EFAULT;
