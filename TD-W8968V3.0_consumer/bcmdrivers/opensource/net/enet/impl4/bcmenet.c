...
#else
static int __my_dev_alloc_name(struct net *net, const char *name, char *buf, int num, int index)
{
	int i = 0;
	const char *p;
...
int my_dev_alloc_name(struct net_device *dev, const char *name, int num, int index)
{
	char buf[IFNAMSIZ];
	struct net *net;
	int ret;
	...
	BUG_ON(!dev_net(dev));
	net = dev_net(dev);
	ret = __my_dev_alloc_name(net, name, buf, num, index);
	if (ret >= 0)
    ...
    for (i = 1, j = 0; i < vport_cnt + 1; i++, j++, map /= 2)
    {
        ...
        else
        {
            if( phy_conn == PHY_CONN_TYPE_PLC ) // Fixme. Use phy_devName in boardparms.
                ...
                my_dev_alloc_name(dev, dev->name, vport_cnt, j);
        }
