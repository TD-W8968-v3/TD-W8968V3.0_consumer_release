...
#define NVRAM_RSA_SIG_LEN 	128
#define NVRAM_FS_KERNEL_LEN 20
...
typedef struct
{
    ...
    unsigned char rsaSig[NVRAM_RSA_SIG_LEN];			/* signature for update */
    unsigned char resRsaSig[NVRAM_RSA_SIG_LEN];			/* reserved for signature */
    unsigned char fs_kernel_len[NVRAM_FS_KERNEL_LEN];	/* length of fs_kernel by str */
    char chUnused[13];
    unsigned char isFactoryTest;             // alloc a byte from chUnused for factory test state by wwj 130517
