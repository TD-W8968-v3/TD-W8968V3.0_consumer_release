...
/* added by wwj 130312 from v4 */
CmsRet cmsTms_getTimeString(UINT32 t, char *buf, UINT32 bufLen)
{
	UINT32 days, hours, minutes, seconds;
    SINT32 r;
    CmsRet ret=CMSRET_SUCCESS;
    ...
    memset(buf, 0, bufLen);
    r = snprintf(buf, bufLen-1, "%dDay(s) %02d:%02d:%02d", days, hours, minutes, seconds);
    if (r >= bufLen)
    {
        ret = CMSRET_RESOURCE_EXCEEDED;
    }
