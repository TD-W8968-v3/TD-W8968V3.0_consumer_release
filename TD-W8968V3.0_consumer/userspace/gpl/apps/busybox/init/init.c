...
extern int prctl_getPidByName(char *name);
extern int cmsFil_writeBufferToFile(const char *filename, const unsigned char *buf, int buflen);

static void wait_for_pid_to_exit(const char *name, int pid, int secs)
{
	int status=0;
	...
	while (secs > 0) {
		...
		// pid has not exited yet, wait for it
		sleep(1);
		secs--;
	}

//	if (secs == 0) {
//		message(L_CONSOLE, "timed out waiting to collect pid %d!!\n", pid);
//		fflush(stdout);
//	}
}
...
static void run_shutdown_and_kill_processes(void)
{
	...
	/* let other apps know that smd is shutting down so they don't print
	 * error messages when they lose connection to smd.
	 */
	cmsFil_writeBufferToFile(SMD_SHUTDOWN_IN_PROGRESS, (unsigned char *) "1", 1);

	/* kill smd first, which will allow most other processes to exit
	 * without having to use signal.
	 */
	pid = prctl_getPidByName("smd");
	kill(pid, SIGKILL);

	if ((pid = prctl_getPidByName("ssk")) > 0) {
		wait_for_pid_to_exit("ssk", pid, 5);
	}

	if ((pid = prctl_getPidByName("mcpd")) > 0) {
		wait_for_pid_to_exit("mcpd", pid, 5);
	}

	if ((pid = prctl_getPidByName("insmod")) > 0) {
		wait_for_pid_to_exit("insmod", pid, 5);
	}

// should try to find extra children of khelper, not the main khelper
//	if ((pid = prctl_getPidByName("khelper")) > 0) {
//		wait_for_pid_to_exit("khepler", pid, 5);
//	}

	/* allow more time for others to exit */
	sleep(1);

	/* Send signals to every process _except_ pid 1 */
	message(L_CONSOLE, "Sending SIG%s to all processes", "TERM");
	kill(-1, SIGTERM);
	sleep(1);
	sync();
	...
	/* umount after all processes have exit. sleep1 to allow umount to work */
