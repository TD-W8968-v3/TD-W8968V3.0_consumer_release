...
{	
	...
	registerInterestedEvent(CMS_MSG_DLNA_INIT);
	registerInterestedEvent(CMS_MSG_DLNA_MANUAL_SCAN);
	registerInterestedEvent(CMS_MSG_DLNA_OP_FOLDER);
	registerInterestedEvent(CMS_MSG_DLNA_RELOAD);
	registerInterestedEvent(CMS_MSG_DLNA_STATE_CHANGE);

	cmsMsg_getEventHandle(msgHandle, &msg_fd);

	/*BCM do not need to do this*/
//	daemonize(1, 0);
