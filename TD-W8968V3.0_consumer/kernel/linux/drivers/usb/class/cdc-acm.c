...
#include <linux/seq_file.h> /* added by zhangshen, 2012-12-13 */
...
static void acm_ctrl_irq(struct urb *urb)
{
	...
	/* Add by zjj, 20121012, from LKML.org, fix potential null-pointer dereference on disconnect. */
	usb_mark_last_busy(acm->dev);
...
exit:
	/* Delete by zjj, 20121012, from LKML.org, fix potential null-pointer dereference on disconnect. */
	//usb_mark_last_busy(acm->dev);
...
static void acm_softint(struct work_struct *work)
{
	...
	if (!ACM_READY(acm))
		return;
	/* Add by zjj, 20121019, from LKML.org, fix potential null-pointer dereference. */
	if (!(acm->tty))
		return;
...
static const struct tty_operations acm_ops = {
	...
	.proc_fops =		&acm_proc_fops, /* added by zhangshen, 2012-12-13 */
};
