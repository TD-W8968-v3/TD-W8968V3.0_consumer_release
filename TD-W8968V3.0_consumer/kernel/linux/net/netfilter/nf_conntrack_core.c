...
nf_ct_clear_blog_bit(void)
{
	...
	printk("nf_ct_clear_blog_bit  begin.........\n");
	
	for (; bucket < nf_conntrack_htable_size; bucket++) 
	{
        write_lock_bh(&nf_conntrack_lock);
		hlist_nulls_for_each_entry_rcu(h, n, &init_net.ct.hash[bucket], hnnode) {
			ct = nf_ct_tuplehash_to_ctrack(h);
			clear_bit(IPS_BLOG_BIT, &ct->status);	/* Disable further blogging */
			if ( (ct->blog_key[IP_CT_DIR_ORIGINAL] != BLOG_KEY_NONE) || 
				(ct->blog_key[IP_CT_DIR_REPLY] != BLOG_KEY_NONE) )
	...
	printk("over my_get_next_corpse:  bucket:0x%x\n", bucket - 1);
