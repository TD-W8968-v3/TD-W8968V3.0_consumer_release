...
int netdev_path_remove(struct net_device *dev)
{
    ...
    /*Hanjiayan@20130927 Add. 
    	  BUG : may cause oops!
      * For a tty device, real device will not be 
      * added to dev->path, so detect NULL string 
      * when undetach the device.
      */	
    if(netdev_path_next_dev(dev))
