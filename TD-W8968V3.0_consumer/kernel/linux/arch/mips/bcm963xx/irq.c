...
#endif
/* add a irq setting function for reset fac default, yuanshang, 2011-09-14 */
/* added from v4.0 sc by wwj 130227 */
void config_fac_default_irq(unsigned int irq, unsigned int high_level);
...
/* ZJin: add a irq setting function for reset fac default. (090216) */
/* add a irq setting function for reset fac default, yuanshang, 2011-09-14 */
/* added from v4.0 sc by wwj 130227 */
void config_fac_default_irq(unsigned int irq, unsigned int high_level)
{
	...
	/* yuanshang: get reset irq */
	if (1)
        {
        /* 96328GW, 963281TAN, BP_EXT_INTR_0(usExtIntrResetToDefault) */
		if ((INTERRUPT_ID_EXTERNAL_0 <= irq) && (INTERRUPT_ID_EXTERNAL_3 >= irq))
		{
			/* ZJin: low_level trigger it. */
			if (0 == high_level)
			{
				printk("irq.c: reset_fac_irq is set to low_trigger.\n");
		        PERF->ExtIrqCfg &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_INSENS_SHFT));    // Edge insesnsitive
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_LEVEL_SHFT));      // Level triggered
		        PERF->ExtIrqCfg &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_SENSE_SHFT));     // Low level
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_CLEAR_SHFT));      // Clear
			...
			/* ZJin: here is high_level */
			else if (1 == high_level)
			{
				printk("irq.c: reset_fac_irq is set to hi_trigger.\n");
		        PERF->ExtIrqCfg &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_INSENS_SHFT));    // Edge insesnsitive
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_LEVEL_SHFT));      // Level triggered
		        //PERF->ExtIrqCfg &= ~(1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_SENSE_SHFT));   
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_SENSE_SHFT));		// Hi level (set bit)
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_CLEAR_SHFT));      // Clear
		        PERF->ExtIrqCfg |= (1 << (irq - INTERRUPT_ID_EXTERNAL_0 + EI_MASK_SHFT));       // Unmask		
			}
	    }
